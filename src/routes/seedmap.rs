...
pub struct SeedmapState {
    ...
    pub tile_cache: Cache<String, Bytes>,
    pub struct_cache: Cache<String, Vec<StructureMarker>>,
}
...
pub async fn tile_handler(
    State(state): State<Arc<SeedmapState>>,
    ...
    let cache_key = format!("{}:{}:{}:{}:{}", seed, q.x, q.z, size, mc);

    let result = state.tile_cache.try_get_with(cache_key, async move {
        tokio::task::spawn_blocking(move || {
            let gen = BiomeGenerator::new(mc, seed, ffi::NO_FLAGS);
            let biomes = gen.get_biomes(q.x >> 2, q.z >> 2, size, size, 4, 320);

            let len = biomes.len();
            let mut buf = bytes::BytesMut::with_capacity(len * 2);
            for &id in &biomes {
                let clamped = id.clamp(-1, 255) as i16;
                buf.extend_from_slice(&clamped.to_ne_bytes());
            }
            Ok::<Bytes, String>(buf.freeze())
...
pub async fn structures_handler(
    State(state): State<Arc<SeedmapState>>,
    Query(q): Query<StructuresQuery>,
) -> impl IntoResponse {
    let seed_str = q.seed.trim().to_string();
    if seed_str.len() > 64 {
        return (StatusCode::BAD_REQUEST, Json(json!({ "error": "seed too long" }))).into_response();
    ...
    let cache_key = format!("{}:{}:{}:{}:{}", seed, q.x, q.z, radius, mc);

    let result = state.struct_cache.try_get_with(cache_key, async move {
        tokio::task::spawn_blocking(move || {
            ...
            Ok::<Vec<StructureMarker>, String>(markers)
        }).await.map_err(|e| e.to_string())?
    }).await;
...
pub fn router(client: reqwest::Client) -> axum::Router {
    ...
    let struct_cache = Cache::builder()
        .time_to_live(Duration::from_secs(3600))
        .max_capacity(1000)
        .build();

    let state = Arc::new(SeedmapState { client, tile_cache, struct_cache });
    axum::Router::new()
